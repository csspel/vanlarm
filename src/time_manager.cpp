...
static time_t timegm_compat(tm *t) {
  ...
  if (oldTz) setenv("TZ", old.c_str(), 1);
  else unsetenv("TZ");
  tzset();
...
static bool parseCclkToEpochUtc(const String& cclk, time_t& outEpochUtc) {
  ...
  if (epochAssumingUtc <= 0) return false;
...
String timeIsoUtc() {
  time_t now = time(nullptr);
  if (now < kMinValidEpoch) return "1970-01-01T00:00:00Z";
...
String timeDateLocal() {
  time_t now = time(nullptr);
  if (now < kMinValidEpoch) return "1970-01-01";
...
String timeClockLocal() {
  time_t now = time(nullptr);
  if (now < kMinValidEpoch) return "00:00:00";
